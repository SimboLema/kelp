...
<head>
    ...
    <title>Edit IPF Plan - Admin Portal</title>
...
<main class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <!-- PAGE HEADER -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Plan — {{ $plan->name }}</h1>
        <p class="text-sm text-slate-500 mt-1">Update parameters and rates for this financing plan.</p>
    </div>

    <!-- EXISTING ACCOUNTS WARNING ALERT -->
    @if ($plan->accounts()->exists())
        <div class="mb-6 rounded-2xl bg-amber-50 border border-amber-200/80 text-amber-800 text-sm p-4 flex items-start gap-3 shadow-sm">
            <svg class="w-5 h-5 text-amber-600 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
            </svg>
            <div>
                <p class="font-semibold text-amber-900">Active accounts linked to this plan</p>
                <p class="mt-0.5 text-amber-700">Changing duration or interest rates will only apply to newly created accounts. Existing customers will retain their original repayment terms and schedule.</p>
            </div>
        </div>
    @endif
